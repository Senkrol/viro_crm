...
class CustomResetPassword extends Notification
{
  ...
  protected function buildMailMessage($url)
  {
    return (new MailMessage)
      ...
      ->line(Lang::get('Вы получили это электронное письмо, потому что был получен запрос на сброс пароля для вашей учетной записи.'))
      ->action(Lang::get('Восстановить пароль'), $url)
      ->line(Lang::get('
Срок действия этой ссылки для сброса пароля истечет через :count минут.', ['count' => config('auth.passwords.' . config('auth.defaults.passwords') . '.expire')]))
